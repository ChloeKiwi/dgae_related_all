...
def degree_stats(graph_ref_list, graph_pred_list, is_parallel=IS_PARALLEL):
...
def spectral_stats(graph_ref_list, graph_pred_list, is_parallel=IS_PARALLEL):
    ''' Compute the distance between the degree distributions of two unordered sets of graphs.
    Args:
      graph_ref_list, graph_target_list: two lists of networkx graphs to be evaluated
    '''
...
def clustering_stats(graph_ref_list, graph_pred_list, bins=100, is_parallel=IS_PARALLEL):
    sample_ref = []
    sample_pred = []
    graph_pred_list_remove_empty = [G for G in graph_pred_list if not G.number_of_nodes() == 0]
