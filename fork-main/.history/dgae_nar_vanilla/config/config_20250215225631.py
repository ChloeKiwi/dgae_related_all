...
def get_prior_config(args):
    ...
    # 使用训练好的autoencoder的yaml，而不是config中的yaml
    config_autoencoder_path = f'./models_own/{args.exp_name}/{args.run_name}/{args.dataset}_autoencoder/config.yaml'
    config_autoencoder_dict = yaml.load(open(config_autoencoder_path, 'r'), Loader=yaml.FullLoader)
    ae_dict ={}
    for key in config_autoencoder_dict:
        if key !='wandb_version' and key!='_wandb':
            ae_dict[key] = config_autoencoder_dict[key]['value']
    config_autoencoder = edict(ae_dict)
    ...
    print(f"prior config: {config}")
    return config

def get_sample_config(args):
    # config_path = args.model_folder
    # config_path = f'./models_own/{args.exp_name}/{args.dataset}_prior/files/config.yaml' #! 生成的config.yaml文件
    # config_path = f'./models_own/{args.exp_name}/{args.dataset}_prior/config.yaml' #! 生成的config.yaml文件
    config_path = f'./models_own/{args.exp_name}/{args.run_name}/{args.dataset}_prior/config.yaml' #! 生成的config.yaml文件
