...
def sample_batch(n_samples, transformer, quantizer, decoder, wandb, max_node_num=None, mask_func=None):
    with torch.no_grad():
        start = time.time()
        transformer.init_sampler(n_samples)
        # zq, masks, indices = sample_prior(n_samples, transformer, quantizer, T=max_node_num//2)
        zq, node_masks, indices = sample_prior_nar_vanilla(n_samples, transformer, quantizer, wandb, T=max_node_num//2, mask_func=mask_func)
