...
def sample_batch(n_samples, transformer, quantizer, decoder, wandb, iterations=None, mask_func=None):
    with torch.no_grad():
        ...
        sample_time = time.time()-start #TODO: sample时间包括：transformer生成token，decoder重构图
...
def sample_prior_nar_vanilla(n_samples, transformer, quantizer, wandb, iterations=None, mask_func=None):
    with torch.no_grad():
        ...
        LARGE_NEGATIVE = -1e9
        # mask_unvalid[..., transformer.mask_token_id] = LARGE_NEGATIVE #mask不能预测
        # mask_unvalid[..., transformer.eos_token] = float('-inf') #eos能预测
        mask_unvalid[..., transformer.sos_token] = LARGE_NEGATIVE #sos不能预测
        # mask_unvalid[..., n_embeddings] = float('-inf') #pad能预测
