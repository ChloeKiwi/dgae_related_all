...
def sample_prior_nar_vanilla(n_samples, transformer, quantizer, T=None, mask_func=None):
    with torch.no_grad():
        ...
        for t in range(T if T is not None else n_max):
            ...
            iter_mask = torch.arange(n_max, device=device)[None, :] < iter_eos_positions[:, None] #bs,n_max
            iter_lengths = torch.sum(iter_mask, dim=1).squeeze(-1) #bs 
            ...
            for batch_idx in range(n_samples):
                iteration_table.add_data(
                    ...
                    iter_mask[batch_idx].cpu().tolist()
                )
        ...
        wandb_table(indices, mask_ratio, mask_len, pre_length)
        # 将indices写入文件
        # write_file(indices)
        return zq, mask.to(device), indices.to(device)

def wandb_table(indices, mask_ratio, mask_len, pre_length):
