...
def get_dataset(dataset: str, config: EasyDict) -> Tuple[List[DataLoader], EasyDict, EasyDict]:
    if dataset == 'zinc' or dataset == 'qm9': #mol数据集
        ...
        if not os.path.exists("./data/stats") or not os.path.exists("./data/data_info"):
    ...
    elif dataset == 'ego' or dataset == 'community' or dataset == 'enzymes' or dataset == 'grid' or dataset == 'grid_1k' or dataset == 'proteins_full' or dataset == 'citeseer' or dataset == 'community_ego' or dataset == 'circular_ladder' or dataset == 'cycle' or dataset == 'ba' or dataset == 'gnp':
        # Choose the appropriate transforms based on the dataset and configuration
        transforms = []
        ...
        if not os.path.exists("./data/data_info/"+dataset+"_stats.txt"):
            print_dataset_statistics(data,dataset)
        ...
        # if data_info.max_node_num < 1000:
        plot_graphs(batch[:20], max_plot=20, wandb=None, title='Original Graph', filepath=f"./plots/{config.exp_name}/{config.run_name}", filename=f'ori_{dataset}', type='png') #原始数据绘制
        # plot_graphs_img(batch[:20], max_plot=20, wandb=None, title='Original Graph', filepath=f"./plots/{config.exp_name}/{config.run_name}", filename=f'ori_{dataset}') #原始数据绘制
        ...
        if dataset=='planar':
            adjs = torch.load(Path("./data/processed")/f"{dataset}_64_200.pt")[0]
        elif dataset=='sbm':
            adjs = torch.load(Path("./data/processed")/f"{dataset}_200.pt")[0]
        data = [nx.from_numpy_array(adj.numpy().astype(bool)) for adj in adjs] #networkx list
        data = [Compose(transforms)(from_networkx(g)) for g in data]
        if dataset=='planar':
            for d in data:
                d.max_num_nodes = 64
        elif dataset=='sbm':
        ...
        torch.manual_seed(SEED)
        idx = torch.randperm(len(data))
        # 将tensor索引转换为Python列表
        train_indices = idx[test_size:].tolist()
        test_indices = idx[:test_size].tolist()
        
        # 创建训练集和测试集的图列表
        train_graphs = [data[i] for i in train_indices]
        test_graphs = [data[i] for i in test_indices]
        
        train_loader = DataLoader(train_graphs, batch_size=config.training.batch_size,
                                shuffle=True, drop_last=True)
