...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model: int, dropout: float = 0, max_len: int = 40):
        ...
        self.register_buffer('pe', pe) #22,1,64
...
class Transformer2d(nn.Module):
    def __init__(self, config, quantizer=None):
        ...
        # out_dim = config.model.quantizer.codebook_size + 4 # for pad(256), mask, sos tokens, eos
        self.d_model = d_model
        self.n_blocks = n_blocks
        ...
        self.tok_emb = nn.Embedding(
            ...
            config.model.quantizer.codebook_size + 4,  # 0-15共16,+pad+sos+eos+mask
            config.transformer.d_model,
            padding_idx=config.model.quantizer.codebook_size  # pad token的索引,使model不更新pad token的embedding
        )
        # self.bias = nn.Parameter(torch.zeros(n_max+1, self.cb_size + 3))
        
        #! quantizer的 codebook embedding
        # self.sos_embedding = nn.Parameter(torch.randn(nc, 1, nz),requires_grad=False).to(device) #(nc,1,nz)
        # self.mask_embedding = nn.Parameter(torch.randn(nc, 1, nz),requires_grad=False).to(device) #(nc,1,nz)
        # self.pad_embedding = nn.Parameter(torch.zeros(nc, 1, nz)).to(device) #(nc,1,nz)
        # self.register_buffer('codebook_embedding', torch.cat((quantizer.embedding, self.pad_embedding, self.sos_embedding, self.mask_embedding), dim=1)) #(nc, cb_size+3, n_z)
        
        # positional encoding
        # self.pe = PositionalEncoding(config.transformer.d_model, dropout=0, max_len=n_max+1) #+sos_token
        self.pe = PositionalEncoding(config.transformer.d_model, dropout=0, max_len=n_max+2) #+sos_token,eos token
    ...
    def forward(self, indices, padding_mask=None):
        """
        Args:
            indices: 已经mask过的token indices (bs, n_max, nc)
            padding_mask: 指示非pad位置的布尔掩码 (bs, seq_len, nc)
        """
        _, _, nc = indices.shape         
        # print("indices: ", indices.squeeze())
        ...
        for c in range(nc):
            ...
            # import ipdb; ipdb.set_trace()
            if padding_mask is not None:
                padding_mask_c = padding_mask[...,c].unsqueeze(-1) #bs,n_max,1
                # emb[c] = emb[c].masked_fill(padding_mask_c == 0, -1e9) #todo 将pad位置的embedding设为很小的值
            # emb[c][..., self.cb_size] = float('-inf')  #todo pad token id
            # emb[c][..., self.cb_size] = 0  #todo pad token id
                
        # # weightying技术
        # for c in range(nc):
        #     emb[c] = torch.matmul(emb[c], self.tok_emb.weight.T) + self.bias #(bs,n_max,n_class)
        
        out = torch.stack(emb, dim=-2) #(32,21,1,259)
