...
class Trainer:
    ...
    def fit_prior(self, batch: Data, train: bool = True) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.config.use_mask and predict_pad:
        ...
        elif self.config.use_mask and predict_wo_eos:
            #既不预测eos，也不计算pad loss
            bs, seq_len, nc = indices.shape
            ## 获取每个序列的实际长度
            lengths = (indices != self.quantizer.n_embeddings).sum(dim=1)[:, 0]  # [bs]
            ...
            #获取有效序列的mask（包括eos，pad）
            # valid_positions = torch.arange(seq_len, device=indices.device)[None, :] <= lengths[:, None]  # [bs, seq_len+1]
            valid_positions = torch.ones(bs, seq_len, nc, device=indices.device, dtype=torch.bool)  # [bs, seq_len, nc]
            ...
            #去除掉pad位置的loss
            pad_mask = target!=self.quantizer.n_embeddings #32,21,1
            loss = (loss * pad_mask.reshape(-1)).sum() / pad_mask.sum()
    ...
    def compute_avg_write_files(self,all_metrics,all_metrics_trainset,all_metrics_novel,all_metrics_novel_trainset,all_sample_time,base_seed=42):
        ...
        with open(result_path, 'w', encoding='utf-8') as f:
            ...
            for i, (m1, m2, m3, m4) in enumerate(zip(all_metrics, all_metrics_trainset, all_metrics_novel, all_metrics_novel_trainset)):
                ...
                f.write(f"  Sample time: {all_sample_time[i]:.4f}\n")
