...
def sample_batch(n_samples, transformer, quantizer, decoder, wandb, iterations=None, mask_func=None):
    with torch.no_grad():
        start = time.time()
        transformer.init_sampler(n_samples)
        # zq, masks, indices = sample_prior(n_samples, transformer, quantizer, T=max_node_num//2)
        zq, node_masks, indices = sample_prior_nar_vanilla(n_samples, transformer, quantizer, wandb, iterations=iterations, mask_func=mask_func)
...
def sample(n_sample, transformer, quantizer, decoder, wandb, iterations=None, mask_func=None):
    if n_sample <= 1000:
        return sample_batch(n_sample, transformer, quantizer, decoder, wandb, iterations, mask_func)
    else:
        K = (n_sample // 1000)
        mod = n_sample % 1000
        annots, adjs = sample_batch(n_sample // K, transformer, quantizer, decoder, iterations, mask_func)
        for k in range(K-1):
...
# maskgit + predict eos
def sample_prior_nar_vanilla(n_samples, transformer, quantizer, wandb, iterations=None, mask_func=None):
    with torch.no_grad():
        # transformer.eval()
        transformer.train()
        n_embeddings = quantizer.n_embeddings
        ...
        # 创建wandb表格
        columns = ["batch_idx", "iteration", "mask_ratio", "mask_len", "pre_length", "indices"]
        iteration_table = wandb.Table(columns=columns)
        
        for t in range(iterations if iterations is not None else n_max):
            # 获取所有位置的预测
            ...
            iteration_table = write_wandb_table(iteration_table,cur_ids, transformer,n_max,device,n_samples,t,mask_ratio,mask_len)
        ...
        # 可视化每一步的mask_ratio,mask_len,pre_length
        wandb.log({'sampling_process': iteration_table})
        # 将indices写入文件
        # write_file(indices)
...
# wo eos  
def sample_prior_nar_vanilla_wo_eos(n_samples, transformer, quantizer, wandb, iterations=None, mask_func=None):
     with torch.no_grad():
        # transformer.eval()
        transformer.train()
        n_embeddings = quantizer.n_embeddings
        ...
        #! 禁止 mask token
        mask_unvalid = torch.zeros((n_samples, n_max+1, nc, cb_size + 3), device=device)
        mask_unvalid[..., transformer.mask_token_id] = float('-inf') #mask不能预测
        # mask_unvalid[..., transformer.eos_token] = float('-inf') #eos不能预测
        # mask_unvalid[..., transformer.sos_token] = float('-inf') #sos不能预测
        # mask_unvalid[..., n_embeddings] = float('-inf') #pad不能预测
        ...
        # 根据第一个PAD位置创建mask
        final_pad_positions = (indices == n_embeddings).float().argmax(dim=1)[:, 0]
        final_pad_positions = torch.where(
            final_pad_positions > 0,
            final_pad_positions,
            torch.tensor(n_max, device=device)
        )
        
        mask = torch.arange(n_max, device=device)[None, :] < final_pad_positions[:, None] #bs,n_max
        
        non_pad_positions = torch.sum(indices != n_embeddings, dim=1) - 1 #bs,nc
        non_pad_positions = torch.clamp(non_pad_positions, min=0)
        
        mask = torch.arange(n_max, device=device)[None, :] < non_pad_positions[:, None] #bs,n_max
...
def write_wandb_table(iteration_table, cur_ids, transformer,n_max,device,n_samples,t,mask_ratio,mask_len):
    # 将当前迭代的结果添加到wandb表格中 
    iter_indices = cur_ids 
    iter_eos_positions = (cur_ids == transformer.eos_token).float().argmax(dim=1)[:, 0]
    iter_eos_positions = torch.where(
        iter_eos_positions > 0,
        iter_eos_positions,
        torch.tensor(n_max, device=device)
    )
    iter_lengths = iter_eos_positions
    ...
    for batch_idx in range(n_samples):
        iteration_table.add_data(
            ...
            # mask_ratio.item(),
            mask_ratio,
            mask_len[batch_idx].item(),
            iter_lengths[batch_idx].item(),
            iter_indices[batch_idx].cpu().tolist(),
        )
