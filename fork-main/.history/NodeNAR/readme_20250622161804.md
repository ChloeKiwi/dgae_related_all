...
modify `--work_type` to run different stages/models.

`--work_type train_autoencoder`: train the GNN encoder, quantizer, and decoder at the first stage.

`--work_type train_prior`: train the transformer at the second stage.

`--work_type sample`: generate new graphs using our trained models.

For example, the following code is used to train the GNN encoder, quantizer, and decoder at the first stage only.
