...
resume_from="f'./models_own/$exp_name/$run_name/{dataset}_prior/best_run_epoch_avg.pth'"
...
run_experiment() {
    ...
    python main.py --work_type train_prior --gpu $gpu --exp_name $exp_name --run_name $exp_config --dataset $dataset --wandb $wandb --codebook_size $codebook_size --nc $nc --resume_from $resume_from 2>&1 | tee "models_own/$exp_name/$exp_config/${dataset}_prior/$date.log"
