...
class Trainer:
    ...
    #     # 在 save_dir 目录下创建 config.yaml 文件
    ...
    def save_config(self, config, save_dir):
        def convert_easydict(obj):
            if isinstance(obj, easydict.EasyDict):
                return dict(obj)
            elif isinstance(obj, dict):
                return {k: convert_easydict(v) for k, v in obj.items()}
            elif isinstance(obj, (list, tuple)):
                return type(obj)(convert_easydict(x) for x in obj)
            return obj

        new_config = {}
        for key,value in config.items():
            value = convert_easydict(value)
            new_config[key] = {'value':value}
    ...
    def compute_avg_write_files(self,all_metrics,all_metrics_trainset,all_metrics_novel,all_metrics_novel_trainset,all_sample_time,base_seed=42):
        ...
        with open(result_path, 'w', encoding='utf-8') as f:
            ...
            f.write(f"\nIndividual run results:\n")
            for i, (m1, m2, m3, m4, m5) in enumerate(zip(all_metrics, all_metrics_trainset, all_metrics_novel, all_metrics_novel_trainset, all_sample_time)):
                seed = base_seed + i
                f.write(f"Run {i+1} (seed {seed}):\n")
                f.write(f"  Sample: {m1}\n")
                ...
                if i!=0:
                    f.write(f"  Sample time: {m5[i-1]:.4f}\n")
