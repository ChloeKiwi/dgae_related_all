...
from utils.logger import log_running_metrics, log_step_autoencoder, save_model, log_mol_metrics, \
    log_step_prior, log_mmd_metrics, load_checkpoint
...
class Trainer:
    def __init__(self, dataloaders, config, data_info):
        ...
        if config.work_type != 'sample':
            ...
            #TODO sweep模式
            with wandb.init(project=project, config=config, mode=config.log.wandb, 
                          name=run_id, resume=False) as run:
                # 使用wandb的配置覆盖原始配置
                ...
                # 更新配置
                config.model.encoder.n_layers = sweep_config.parameters.model.encoder.n_layers
                config.model.encoder.nhf = sweep_config.model.encoder.nhf 
                config.model.encoder.mlp_hidden_size = sweep_config.model.encoder.mlp_hidden_size
                config.model.encoder.mlp_n_layers = sweep_config.model.encoder.mlp_n_layers
                config.model.decoder.n_layers = config.model.encoder.n_layers
                config.model.decoder.nhf = config.model.encoder.nhf
                config.model.decoder.mlp_hidden_size = config.model.encoder.mlp_hidden_size
                config.model.decoder.mlp_n_layers = config.model.encoder.mlp_n_layers
                config.model.quantizer.codebook_size = sweep_config.model.quantizer.codebook_size
                config.model.quantizer.nc = sweep_config.model.quantizer.nc
                config.model.quantizer.init_steps = sweep_config.model.quantizer.init_steps
                config.training.learning_rate = sweep_config.training.learning_rate
    ...
    def load_from_checkpoint(self, checkpoint_path):
        """加载检查点继续训练"""
        if self.config.work_type == 'train_autoencoder':
            models = (self.encoder, self.decoder, self.quantizer)
        else:  # train_prior
            models = (self.transformer,)
            
        step, self.best_run = load_checkpoint(
            checkpoint_path,
            models,
            self.opt,
            self.scheduler
    ...
    def compute_recon_mmd(self):
        ...
        print("Recon std metrics:", std_metrics)
         ...
         # 将结果写入文件
        date=datetime.now().strftime("%Y%m%d-%H%M")
        result_path = f'./models_own/{self.config.exp_name}/{self.config.run_name}/{self.config.dataset}_sample/recon_metrics_{date}.txt'
    ...
    def autoencoder(self) -> None:
        ...
        if hasattr(self.config, 'resume_from') and self.config.resume_from:
            step = self.load_from_checkpoint(self.config.resume_from)
        ...
        for epoch in tqdm(range(1, self.epochs)):
            ...
            for batch in self.train_loader:
                ...
                if step % self.n_logging_steps == 0:
                    ...
                    annots, adjs,_ ,_ = sample_batch(self.n_samples, self.transformer, self.quantizer, self.decoder)
            ...
            if epoch % self.n_logging_epochs == 0:
                ...
                annots, adjs, indices, sample_time = sample(self.n_samples, self.transformer, self.quantizer, self.decoder)
                if self.mol_data:
