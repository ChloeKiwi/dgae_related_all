...
def orca(graph):
    ...
    print(f'orca tmp_file_path: {tmp_file_path}')
...
def orca(graph):
    # 使用进程ID创建唯一的临时文件名
    pid = os.getpid()
    tmp_file_path = os.path.join(ORCA_DIR, f'tmp_{pid}.txt')
    ...
    try:
        ...
        try:
            ...
            print(f"清理临时文件 {tmp_file_path} 时发生错误: {e}")
