...
def log_step_autoencoder(metrics, batch, rec_losses, vq_losses, masks, n_node_feat, train, annotated_nodes):
    ...
    metric_values = (loss.item(), edge_loss.item(), edge_acc.item(), n_graph_rec.item(),
                     codebook.item(), commit.item(), perplex, recon_loss.item())
...
def load_checkpoint(checkpoint_path, models, optimizer, scheduler=None):
    print(f'Loading checkpoint from {checkpoint_path}')
    checkpoint = torch.load(checkpoint_path)
    import ipdb; ipdb.set_trace()
    if len(models) == 3:
        encoder, decoder, quantizer = models 
    ...
    print(f"成功加载检查点,从第 {start_iteration} 步继续训练")
    return start_iteration, best_metrics
...
def log_mmd_metrics(batch, adjs, key):
    ...
    metrics, _ = eval_torch_batch(refs, adjs)
