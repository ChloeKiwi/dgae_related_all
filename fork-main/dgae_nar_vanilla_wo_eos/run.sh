...
copy_wandb_files() {
    ...
    if [ -n "$run_id" ]; then
        # echo "复制 ${work_type} 的wandb文件 (Run ID: ${run_id})..."w
...
exp_name='dgae_mlm'
# run_name='community_cb16'
run_name='community_cb16_no_sort'
# run_name='community_cb256'
# run_name='citeseer_cb16_2'
# run_name='community_cb256_init100'
...
run_dataset() {
    ...
    python main.py --work_type sample --gpu $gpu --exp_name $exp_name --run_name $run_name --dataset $dataset --use_mask 2>&1 | tee "models_own/$exp_name/$run_name/${dataset}_sample/$date.log"
