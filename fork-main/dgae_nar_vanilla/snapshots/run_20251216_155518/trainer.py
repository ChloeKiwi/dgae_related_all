...
class Trainer:
    def __init__(self, dataloaders, config, data_info):
        ...
        if config.train_prior and config.training.sort_indices:
            self.sort_indices = utils.func.sort_indices
        elif config.train_prior and not config.training.sort_indices:
            self.sort_indices = None
        # for mlm
        pad_token = self.cb_size #128
        self.use_mask = config.use_mask
        self.sos_token = self.cb_size + 1 #129
        self.eos_token = self.cb_size + 2 #130
        self.mask_token_id = self.cb_size + 3 #131
        self.mask_gamma = self.gamma_func(config.mask_func)
        self.iterations_rate = config.iterations_rate
        self.iterations = int(self.iterations_rate * config.data.max_node_num)
        print(f"use_mask: {self.use_mask}, iterations: {self.iterations}, iterations_rate: {self.iterations_rate}, sos_token: {self.sos_token}, eos_token: {self.eos_token}, mask_token_id: {self.mask_token_id}, nc: {self.nc}, wandb:{config.log.wandb}")
        ...
        run_id = f'{config.exp_name}_{config.run_name}_{work_type}'
        ...
        ### re-save config ###
        if config.work_type=='train_autoencoder':
            model_save_dir=f'./models_own/{self.config.exp_name}/{self.config.run_name}/{self.config.dataset}_autoencoder'
        elif config.work_type=='train_prior':
            model_save_dir=f'./models_own/{self.config.exp_name}/{self.config.run_name}/{self.config.dataset}_prior'
        elif config.work_type=='sample':
            model_save_dir=f'./models_own/{self.config.exp_name}/{self.config.run_name}/{self.config.dataset}_sample'
        # import ipdb; ipdb.set_trace()
        self.save_config(self.config,model_save_dir)
    ...
    def save_config(self,config,save_dir):
        def convert_easydict(d):
            if isinstance(d, easydict.EasyDict):
                return {k: convert_easydict(v) for k, v in dict(d).items()}
            elif isinstance(d, dict):
                return {k: convert_easydict(v) for k, v in d.items()}
            return d
    
        new_config = {}
        for key, value in config.items():
            new_config[key] = {'value': convert_easydict(value)} 
        
        additional_config = {
            'dataset':{'value':self.config.dataset},
            'device':{'value':str(self.config.device)},
            'exp_name':{'value':self.config.exp_name},
            'run_name':{'value':self.config.run_name}            
        }
        new_config.update(additional_config)
        ...
        for epoch in tqdm(range(1, self.epochs)):
            ...
            for batch in self.train_loader:
                ...
                if step % self.n_logging_steps == 0:
                    ...
                    annots, adjs, indices, sample_time = sample_batch(self.n_samples, self.transformer, self.quantizer, self.decoder, self.wandb ,self.iterations, self.config.mask_func)
            ...
            if epoch % self.n_logging_epochs == 0:
                ...
                annots, adjs, indices, sample_time = sample(self.n_samples, self.transformer, self.quantizer, self.decoder, self.wandb, self.iterations, self.config.mask_func)
                if self.mol_data:
    ...
    def fit_prior(self, batch: Data, train: bool = True) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # predict_pad = False
        # predict_eos = False
        # predict_eos_pad = False #不过滤pad，预测unmask位置
        # predict_mask_ignore_pad= False #mask和unmask位置都预测，过滤pad
        # predict_mask_bert = True #过滤pad，只预测mask位置
        if self.config.use_mask and self.config.predict_method == 'predict_pad':
        ...
        elif self.config.use_mask and self.config.predict_method == 'predict_eos':
            bs, seq_len, nc = indices.shape
            ## 获取每个序列的实际长度
            ...
            # total_loss = F.cross_entropy( #不计算mask
            #     logits[:,:,:,:].reshape(-1,logits.size(-1)),  # (bs * seq_len * nc, 259)
            #     target[:,:,:].reshape(-1),                    # (bs * seq_len * nc)
            #     reduction='none'
            # )
            
            #  # 检查 total_loss 是否有 nan
            # if torch.isnan(total_loss).any():
            #     # import ipdb; ipdb.set_trace()
            #     print("Found nan in total_loss!")
            #     print("logits contains nan:", torch.isnan(logits).any())
            
            # pad_positions = (target == self.quantizer.n_embeddings).reshape(-1)
            # mask_positions = (a_indices == self.mask_token_id).reshape(-1)
            # # 验证 pad 位置和 mask 位置没有重叠
            # assert not torch.any(pad_positions & mask_positions), "Found mask in padding position!"
            
            # # 计算mask部分的losstotal_loss
            # # mask_positions = (a_indices == self.mask_token_id).reshape(-1) #mask位置,pad一定为0
            # loss = (total_loss * mask_positions.float()).sum() / mask_positions.sum()
            # 计算pad部分的loss
            # pad_loss = (total_loss * pad_mask).sum() / pad_mask.sum()
        
        elif self.config.use_mask and self.config.predict_method == 'predict_mask_ignore_pad':
            bs, seq_len, nc = indices.shape
            ## 获取每个序列的实际长度
            lengths = (indices != self.quantizer.n_embeddings).sum(dim=1)[:, 0]  # [bs]
            ...
            total_loss = F.cross_entropy( #不计算mask
                logits[:,:,:,:].reshape(-1,logits.size(-1)),  # (bs * seq_len * nc, 259)
                target[:,:,:].reshape(-1),                    # (bs * seq_len * nc)
                ignore_index=self.quantizer.n_embeddings #pad位置不计算loss
            )
        ...
        elif self.config.use_mask and self.config.predict_method == 'predict_mask_bert':
            bs, seq_len, nc = indices.shape
            ## 获取每个序列的实际长度
            lengths = (indices != self.quantizer.n_embeddings).sum(dim=1)[:, 0]  # [bs]
            ...
            # 在有效位置中进行mask（包括内容和EOS）
            # num_mask = math.floor(self.mask_gamma(np.random.uniform()) * seq_len) #向下取，可能为0
            num_mask = math.ceil(self.mask_gamma(np.random.uniform()) * seq_len) #向上取，最小为1
            mask_scores = torch.rand(modified_indices.shape, device=indices.device)
            mask_scores = mask_scores.masked_fill(~valid_positions, float('-inf'))  #将pad位置的分数设为负无穷
            random_indices = mask_scores.topk(k=min(num_mask, lengths.max()), dim=1).indices #要被mask的索引
            ...
            # --- Standard BERT MLM Loss ---
            # 1. Construct mask for target (prepend False for SOS)
            sos_mask = torch.zeros(bs, 1, nc, dtype=torch.bool, device=indices.device)
            full_mask = torch.cat((sos_mask, mask), dim=1) 

            # 2. Create labels: -100 for unmasked, Real ID for masked
            labels = target.clone()
            labels[~full_mask] = -100  # 非mask部分的label改为-100，为了不计算loss

            # 3. Calculate Loss only on masked positions
            loss = F.cross_entropy(
                logits.view(-1, logits.size(-1)),
                labels.view(-1),
                ignore_index=-100 #只计算mask部分
            )
        ...
        elif self.config.use_mask and self.config.predict_method == 'predict_eos_pad': #pad位置也可以被mask，预测unmask位置
            bs, seq_len, nc = indices.shape
            ## 获取每个序列的实际长度
            lengths = (indices != self.quantizer.n_embeddings).sum(dim=1)[:, 0]  # [bs]

            # indices矩阵长度+1，为了放置EOS token
            modified_indices = torch.ones(bs, seq_len + 1, nc,  #bs,n_max+1,nc
                                        dtype=torch.long, 
                                        device=indices.device) * self.quantizer.n_embeddings  # 用pad token初始化
            ...
            for b in range(bs):
                length = lengths[b]
                if length < seq_len:
                    modified_indices[b, length, :] = self.eos_token # 长度小于n_max时，在长度位置放EOS token
                else:  # 序列长度等于n_max的情况
                    modified_indices[b, seq_len, :] = self.eos_token  # 在新增的位置放EOS
            ...
            # 获取有效序列
            # import ipdb; ipdb.set_trace()
            valid_positions = torch.arange(seq_len, device=indices.device)[None, :] <= lengths[:, None]  # [bs, n_max+1] pad位置不能被mask掉
            valid_positions = valid_positions.unsqueeze(-1)  # [bs, n_max+1, 1] 
            # valid_positions = torch.ones(bs, seq_len, nc, device=indices.device, dtype=torch.bool)  # [bs, n_max+1, nc] 全部为True，包括pad位置也可以被mask掉
            ...
            # check num_mask>0
            if num_mask<=0:
                print("num_mask is <=0:", num_mask)
            
            mask_scores = torch.rand(modified_indices.shape, device=indices.device) # 生成形状和indices矩阵相同的随机分数
            mask_scores = mask_scores.masked_fill(~valid_positions, float('-inf'))  # 将不能被mask的位置分数设为负无穷
            random_indices = mask_scores.topk(k=min(num_mask, lengths.max()), dim=1).indices #生成形状和indices矩阵相同的随机索引，为了每条数据mask的位置不同
            ...
            # 拼接序列
            sos_tokens = torch.ones(bs, 1, nc, dtype=torch.long, device=indices.device) * self.sos_token #32,1,1
            a_indices = torch.cat((sos_tokens, masked_indices), dim=1) #32,22,1
            target = torch.cat((sos_tokens, modified_indices), dim=1) #32,22,1
            
            logits = self.transformer(a_indices) 
            
            # 计算unmask位置的loss
            # loss = F.cross_entropy( #不计算mask
            #     logits[:,:,:,:].reshape(-1,logits.size(-1)),  # (bs * seq_len * nc, 259)
            #     target[:,:,:].reshape(-1),                    # (bs * seq_len * nc)
            #     ignore_index=self.mask_token_id           # 忽略 mask id 
            # )
            ...
            if num_masked > 0:
                loss = (loss * mask_positions.float()).sum() / num_masked
            else:
                print("num_masked <=0:", num_masked)
        ...
        # if torch.isnan(loss):
        #     print("final loss is nan, not backpropagate")
        #     return
        # else:
        # if train:
        #     loss.backward()
        ...
        with torch.no_grad():
            ...
            if self.mol_data:
                ...
                for run in range(n_runs):
                    ...
                    start = time.time()
                    annots, adjs,indices = sample(self.n_samples, self.transformer, self.quantizer, self.decoder, self.wandb, self.iterations, self.config.mask_func)
                ...
                base_seed = 42
                # base_seed = 3407
                all_metrics, all_metrics_trainset, all_metrics_novel, all_metrics_novel_trainset,all_sample_time = [],[],[],[],[]
                
                with torch.cuda.amp.autocast():  # 使用混合精度
                    for run in range(n_runs):
                        # 为每次运行设置不同的随机种子
                        # current_seed = base_seed + run
                        current_seed = base_seed + math.floor(np.random.uniform() * 100000)
                        ...
                        for batch in tqdm(self.test_loader, desc='Sampling...'):
                            annots, adjs, indices, sample_time = sample(self.n_samples, self.transformer, self.quantizer, self.decoder, self.wandb, self.iterations, self.config.mask_func)
                            ...
                            # 只在第一次运行时绘图
                            if run == 0:
                                plot_graphs(adjs[:20], max_plot=20, wandb=None, 
                                            # title='Generated Graph', 
                                            filepath=f"./plots/{self.config.exp_name}/{self.config.run_name}", 
                                            filename=f"{self.config.dataset}_sample_graph", indices=None)
                                plot_graphs(adjs_trainset[:20], max_plot=20, wandb=None, 
                                            # title='Trainset Graph', 
                                            filepath=f"./plots/{self.config.exp_name}/{self.config.run_name}", 
                            ...
                            if run!=0: #去掉第一次的sample time
                                all_sample_time.append(sample_time)
                                print(f"Run {run+1} (seed {current_seed}) sample time:", sample_time)
                    ...
                    self.compute_avg_write_files(all_metrics,all_metrics_trainset,all_metrics_novel,all_metrics_novel_trainset,all_sample_time,base_seed)
                    
    def compute_avg_write_files(self,all_metrics,all_metrics_trainset,all_metrics_novel,all_metrics_novel_trainset,all_sample_time,base_seed=42):
        date=datetime.now().strftime("%Y%m%d-%H%M")
        # 计算平均指标
        avg_metrics = {}
        ...
        # print("Sample time:", np.mean(all_sample_time))
                        
        # 将结果写入文件
        result_path = f'./models_own/{self.config.exp_name}/{self.config.run_name}/{self.config.dataset}_sample/metrics_{date}.txt'
        os.makedirs(os.path.dirname(result_path), exist_ok=True)
        ...
        with open(result_path, 'w', encoding='utf-8') as f:
            ...
            f.write(f"\nSample time:\n")
            f.write(f"  {np.mean(all_sample_time):.4f}\n")
