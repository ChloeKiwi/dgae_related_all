...
# exp_name=$exp_name"_"$(date +%Y%m%d-%H)
...
declare -A experiment_configs=(
    ...
    # ['community_128_1_ignore_pad_cosine_bert_mask_type']="community 128 1 0 cosine"
    # ['community_128_1_ignore_pad_cosine_bert_mask_type']="community 128 1 0 cosine 1"
    # ['community_128_1_not_ignore_pad_cosine_predict_eos_pad_cal_mask_loss']="community 128 1 0 cosine 1"
    # ['community_128_1_ignore_pad_cosine_predict_eos_pad_cal_mask_loss_seed3407']="community 128 1 3 cosine 1"
    ['community_128_1_ignore_pad_cosine_predict_eos_pad_cal_mask_loss']="community 128 1 0 cosine 1 predict_eos_pad"
    ['community_128_1_ignore_pad_cosine_predict_bert_mask_loss']="community 128 1 3 cosine 1 predict_mask_bert"
    # ['community_128_1_ignore_pad_cosine_predict_eos_cal_mask_loss']="community 128 1 0 cosine 1"
    ...
    # ['community_256_1']="community 256 1 2"z
    ...
    # ["proteins_full_cb512_2"]='proteins_full 512 2 2'
    # ["proteins_full_cb2048_2"]='proteins_full 2048 2 1'
...
# 创建所有必要的目录
for original_key in "${!experiment_configs[@]}"; do
    # exp_config="${original_key}_${date}"
    exp_config="${original_key}" #不加date
    read -r dataset _ _ _ _ _ <<< "${experiment_configs[$original_key]}"
...
run_experiment() {
    local exp_config=$1
    # local original_key=${exp_config%_$date}
    local original_key=${exp_config} #不加date
    read -r dataset codebook_size nc gpu mask_func iterations_rate predict_method<<< "${experiment_configs[$original_key]}"
    
    echo "开始实验配置: $original_key (数据集: $dataset, GPU: $gpu, Codebook Size: $codebook_size, NC: $nc, Mask Function: $mask_func, Iterations Rate: $iterations_rate, Predict Method: $predict_method)"
    ...
    python main.py --work_type train_prior --gpu $gpu --exp_name $exp_name --run_name $exp_config --dataset $dataset --wandb $wandb --codebook_size $codebook_size --nc $nc --use_mask --mask_func $mask_func --iterations_rate $iterations_rate --predict_method $predict_method 2>&1 | tee "models_own/$exp_name/$exp_config/${dataset}_prior/$date.log"
    echo "[$exp_config] train_prior 完成"


    echo "[$exp_config] sample 开始..."
    python main.py --work_type sample --gpu $gpu --exp_name $exp_name --run_name $exp_config --dataset $dataset  --wandb $wandb --codebook_size $codebook_size --nc $nc --use_mask --mask_func $mask_func --iterations_rate $iterations_rate --predict_method $predict_method 2>&1 | tee "models_own/$exp_name/$exp_config/${dataset}_sample/$date.log"
