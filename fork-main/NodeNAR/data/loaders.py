...
class FromNetworkx(InMemoryDataset):
    ...
    def raw_file_names(self):
        ...
        if self.dataset == 'citeseer':
            ...
            return ['grid.pkl']
        elif self.dataset == 'grid_1k':
            return ['grid_1k.pkl']
        elif self.dataset == 'proteins_full':
            return ['PROTEINS_full.pkl']
        elif self.dataset == 'planar':
            return ['planar.pkl']
        elif self.dataset == 'sbm':
            return ['sbm.pkl']
        elif self.dataset == 'circular_ladder':
            return ['circular_ladder.pkl']
        elif self.dataset == 'cycle':
            return ['cycle.pkl']
        elif self.dataset == 'ba':
            return ['ba.pkl']
        elif self.dataset == 'gnp':
            return ['gnp.pkl']
        else:
            raise NotImplementedError()
    ...
    def process(self):
        ...
        for g in graph_list:
            ...
            if self.dataset == 'ego':
                ...
                data.max_num_nodes = 1089
            elif self.dataset == 'proteins_full':
                # data.max_num_nodes = 1575
                data.max_num_nodes = 1396
            elif self.dataset == 'citeseer':
                data.max_num_nodes = 18
            elif self.dataset == 'circular_ladder':
                data.max_num_nodes = 496
            elif self.dataset == 'cycle':
                data.max_num_nodes = 495
            elif self.dataset == 'ba':
                data.max_num_nodes = 99
            elif self.dataset == 'gnp':
                data.max_num_nodes = 96
            else:
                raise NotImplementedError
            data_list.append(data)
        data, slices = self.collate(data_list)
