...
class Trainer:
    ...
    def precess_input(self, indices):
        # replace the 1st and 2nd [pad] to [eos]
        print(indices)
        bs, seq_len, nc = indices.shape 
        lengths = (indices != self.quantizer.n_embeddings).sum(dim=1)[:,0] #bs 
        # 创建一个比原序列多一个位置的tensor
        input_indices = torch.ones(bs, seq_len + 2, nc,  #bs,n_max+2,nc
                                    dtype=torch.long, 
                                    device=indices.device) * self.quantizer.n_embeddings  # 用pad token初始化
        # 复制原始序列
        input_indices[:, :seq_len, :] = indices #bs,n_max+2,nc
        # 对每个batch样本处理
        for b in range(bs):
            length = lengths[b]
            if length < seq_len:  # 有pad token的情况
                input_indices[b, length, :] = self.eos_token
                input_indices[b, length+1, :] = self.eos_token #!加了2个eos
            else:  # 序列长度等于n_max的情况
                input_indices[b, seq_len, :] = self.eos_token  # 在新增的位置放EOS
                input_indices[b, seq_len+1, :] = self.eos_token #!第二个eos
        
        return input_indices #bs, seq_len+2, nc
    ...
    def fit_prior(self, batch: Data, train: bool = True) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        zq, masks, indices = dense_zq(zq, indices, self.quantizer, batch, self.max_node_num, self.sort_indices) #zq: bs,n_max,nc,d  indices:bs,n_max,nc
        bs, seq_len, nc = indices.shape
        processed_indices = self.precess_input(indices)
        # 拼接序列
        sos_tokens = torch.ones(bs, 1, nc, dtype=torch.long, device=indices.device) * self.sos_token
        a_indices = torch.cat((sos_tokens, processed_indices), dim=1)
        target = processed_indices
