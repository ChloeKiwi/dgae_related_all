...
class Decoder(nn.Module):
    ...
    def forward(self, node_feat, mask=None): #node_feat: bs,n_max,nc*nz
...
class Encoder(nn.Module):
    def __init__(self, config, data_info):
        ...
        mlp_hidden_size = config.model.encoder.mlp_hidden_size #64
    ...
    def forward(self, batch):
        ...
        # batch.x:2048,30; edge_index_ext:2,63206; edge_attr_ext:63206,3
...
class Transformer2d(nn.Module):
    ...
    def forward(self, embeddings, mask=None):
        ...
        v = self.mlp_inV(z[0])
        k = self.mlp_inK(z[0])
        v = self.pe(v.permute(1, 0, 2)).permute(1, 0, 2) #!no pe
        for c in range(nc):
    ...
    def sample(self, z_c, c, z_completed):
        ...
        v0 = self.pe(self.Vs[0].permute(1, 0, 2)).permute(1, 0, 2) #!no pe
        # v0 = self.Vs[0] #!no pe
 ...
 class Transformer2d(nn.Module):
    def __init__(self, config, quantizer=None):
        ...
        num_heads = config.transformer.num_heads
        n_blocks = config.transformer.n_blocks
        out_dim = config.model.quantizer.codebook_size + 2 #add pad, eos
        self.d_model = d_model
        self.n_blocks = n_blocks
        
        self.cb_size = config.model.quantizer.codebook_size
        # self.sos_token = self.cb_size + 1
        self.eos_token = self.cb_size + 1
        self.padding_idx = config.model.quantizer.codebook_size
        # 新token embedding层
        self.tok_emb = nn.Embedding(
            # config.model.quantizer.codebook_size + 3,  # +3 for pad(256), mask, sos tokens,259
            config.model.quantizer.codebook_size + 2,  # 0-15共16,+pad+sos+eos+mask
            config.transformer.d_model,
            padding_idx=self.padding_idx  # pad token的索引
        )
        ...
        self.n_max = n_max
        self.out_dim =out_dim
        self.pe = PositionalEncoding(d_model, dropout=0, max_len=n_max+1) #+ eos 

        self.mlp_in = nn.ModuleList([])
        for c in range(nc):
            # self.mlp_in.append(nn.Linear(nz + nz * c // nc, d_model))
            self.mlp_in.append(nn.Linear(nz, d_model))
        ...
        self.toplayer = TopLayer(self.cb_size, self.d_model, crf_low_rank, crf_beam_size, dropout, self.padding_idx)

    
    def get_codebook_embedding(self, indices, c_idx):
        """
        Args:
            indices: (bs, n_max+1)
        Returns:
            embeddings: (bs, n_max+1, nc, d_model)
        """
        embeddings = F.embedding(indices, self.codebook_embedding[c_idx]) #(bs, n_max+1, d_model)
        return embeddings
    ...
    def forward(self, indices):
        ...
        z, q, emb = [None] * (nc), [None] * (nc), [None] * (nc) 
        log_probs, batch_crf_loss = [None] * (nc), [None]*(nc)
         # 2. 处理每个通道
        for c in range(nc):
            # 使用quantizer的codebook embedding
            # z[c] = self.get_codebook_embedding(indices[..., c], c) #获取c通道的codebook embedding (bs, n_max+1, nz)
            # z[c] = self.mlp_in[c](z[c]) #(bs, n_max+1, d_model)
            
            # 使用可训练的embedding层
            z[c] = self.tok_emb(indices[..., c])  # (bs, n_max, d_model)
            padding_mask = ~indices[:,c].eq(self.padding_idx)
        ...
        for i, layer in enumerate(self.layers):
            ...
            for c in range(nc):
                q[c] = self.mlp_q[i][c](emb[c])
                emb[c] = layer[c](q[c], k, v, emb[c], normalize_output=True) #bs,n_max,d_model

        
        # # 5. 输出层
        # for c in range(nc):
        #     emb[c] = self.mlp_out[c](emb[c]) #d_model -> out_dim
        
        assert emb[c].size() == torch.size([bs, n_max, self.d_model])
        log_probs[c], batch_crf_loss[c] = self.toplayer(emb[c], indices[...,c], is_training=True)
        
        print(log_probs)
        print(batch_crf_loss)
...
class TopLayer(nn.Module):
    def __init__(self, vocab_size, embed_dim, crf_low_rank, crf_beam_size, dropout, padding_idx):
        ...
        self.crf_layer = DynamicCRF(num_embedding = vocab_size, low_rank = crf_low_rank, 
                                    beam_size = crf_beam_size) #! crf layer

        self.one_more_layer_norm = nn.LayerNorm(embed_dim)
        self.tgt_word_prj = nn.Linear(self.embed_dim, self.vocab_size)
    
    def forward(self, rep, indices, is_training):
        indices = indices.transpose(0,1) #seq_len, bs
        seq_len, bs = indices.size()
        
        rep = F.dropout(rep, p=self.dropout, training=is_training)
        rep = rep.transpose(0, 1)
        
        emissions = self.tgt_word_prj(rep.contiguous().view(-1, self.embed_dim)).view(seq_len, bs, self.vocab_size)
        ...
        emission_mask = ~indices.eq(self.padding_idx)
        batch_crf_loss = -1 * self.crf_layer(emissions, indices, emission_mask) # [bsz]
    ...
    def decoding(self, src_representation, src_input):
        ...
        emissions = self.tgt_word_prj(src.contiguous().view(-1, self.embed_dim)).view(seqlen, bsz, self.vocab_size) #! 将src展平，然后通过线性层，得到每个token的logits

        emissions = emissions.transpose(0, 1) # [bsz, seqlen, vocab_size]
        _, finalized_tokens = self.crf_layer.forward_decoder(emissions) #! crf decoding
