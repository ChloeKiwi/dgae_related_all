...
# exp_name='dgae_baseline_different_hyper'
exp_name='community_scaling_law_2'
...
declare -A experiment_configs=(
    ...
    ['community_200']='community_200 0'
    ['community_500']='community_500 2'
    ['community_1000']='community_1000 3'
    ['community_2000']='community_2000 0'

)

# 创建所有必要的目录
for exp_config in "${!experiment_configs[@]}"; do
    read -r dataset _ <<< "${experiment_configs[$exp_config]}"
...
run_experiment() {
    local exp_config=$1
    read -r dataset gpu <<< "${experiment_configs[$exp_config]}"
    
    echo "开始实验配置: $exp_config (数据集: $dataset, GPU: $gpu"
