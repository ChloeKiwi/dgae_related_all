...
def get_dataset(dataset: str, config: EasyDict) -> Tuple[List[DataLoader], EasyDict, EasyDict]:
    if dataset == 'zinc' or dataset == 'qm9': #mol数据集
    ...
    elif dataset == 'ego' or dataset == 'community' or dataset == 'enzymes' or dataset == 'grid' or dataset == 'protein' or dataset == 'citeseer' or dataset=='community_200' or dataset=='community_500' or dataset=='community_1000' or dataset=='community_2000':
        # Choose the appropriate transforms based on the dataset and configuration
        transforms = []
        ...
        #todo:标记结构相似的节点组，无法添加新属性，有bug
        # data = mark_structural_groups(data, 
        #                             radius=1,  # 可以通过config配置
        #                             ged_threshold=1)  # 可以通过config配置
