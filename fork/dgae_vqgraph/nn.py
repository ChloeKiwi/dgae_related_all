...
class Gnn(nn.Module):
    ...
    def forward(self, node_feat, edge_feat=None, skip_connection=False):
        nodes2nodes = nodes2edges(node_feat) #nodes2nodes:node_feat
...
class GnnSparse(MessagePassing):
    def __init__(self, nnf_in, nef_in, nnf_out, nef_out, n_layers, hidden_size, normalization=None):
        ...
        self.normalization = normalization
        self.edge_feat = 0 #!
        # print("gnn_sparse params: nnf_in: {}, nef_in: {}, nnf_out: {}, nef_out: {}, n_layers: {}, hidden_size: {}, normalization: {}".format(nnf_in, nef_in, nnf_out, nef_out, n_layers, hidden_size, normalization))
    ...
    def forward(self, x, edge_index, edge_attr=None, skip_connection=False):
        ...
        if skip_connection: #如果进行跳跃连接
            node_feat = node_feat + x # 将传播后的节点特征与输入的节点特征相加

        if self.normalization == 'batch_norm': #如果进行批归一化
            self.edge_feat = self.edge_norm(self.edge_feat) #对边特征进行归一化
            node_feat = self.node_norm(node_feat) #对节点特征进行归一化
        elif self.normalization == 'layer_norm': #如果进行层归一化
            self.edge_feat = self.edge_norm(self.edge_feat) #对边特征进行归一化
            node_feat = self.node_norm(node_feat) #对节点特征进行归一化
        return node_feat, self.edge_feat #返回传播后的节点特征和边特征
