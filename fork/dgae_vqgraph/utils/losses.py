...
def get_unannotated_loss_and_rec(batch, edges_rec, edge_masks, max_node_num):
    ...
    # edges_rec = edges_rec * edge_masks.squeeze() #TODO: edge_mask是4维，因为edges_rec表示batch_size*max_num_nodes*max_num_nodes*n_edge_feat
    edges_rec = edges_rec * edge_masks #TODO: edge_mask是4维，因为edges_rec表示batch_size*max_num_nodes*max_num_nodes*n_edge_feat
    edge_loss = F.binary_cross_entropy(edges_rec.flatten(1), adjs.flatten(1), reduction='mean')
    edges_rec = edges_rec.round() #将边特征进行四舍五入 #TODO: use threshold or round?
    return edge_loss, edges_rec