...
class Trainer:
    def __init__(self, dataloaders, config, data_info):
        ...
        print(f"use_mask: {self.use_mask}, use vq: {self.quantization}, sos_token: {self.sos_token}, mask_token_id: {self.mask_token_id}, cb_size: {self.cb_size}, nc: {self.nc}, wandb:{config.log.wandb}, is mol:{self.mol_data}, recon nodes:{self.annotated_nodes}")
    ...
    def fit_autoencoder(self, batch: Data, train: bool = True) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.init_steps > 0:
            ...
            if self.quantization:
                zq, commit, codebook, perplex, indices = self.quantizer(ze) #go into vq
        ...
        with torch.no_grad():
            for batch in self.train_loader:
                ...
                if ref.shape[0]>=self.n_samples:
                    ...
                    break
        print(f"have collected {ref.shape[0]} trainset graphs")
        ...
        for epoch in tqdm(range(1, self.epochs)):
            ...
            for batch in self.train_loader:
                ...
                if step % self.n_logging_steps == 0:
                    ...
                    annots, adjs = sample_batch(self.n_samples, self.transformer, self.quantizer, self.decoder, T=10, use_mlm=self.use_mask) #生成样本
                    ...
                    # print(f"prior mmd at train epoch {epoch}: {gen_metrics}")
            ...
            if epoch % self.n_logging_epochs == 0:
                ...
                # # 计算与baseline mmd差值
                # prior_mmd_difference = abs(gen_metrics['avg']-gen_metrics_baseline['avg'])
                # wandb.log({'prior mmd diff': prior_mmd_difference})
    ...
    def sample(self) -> None:
        ...
        # sample_feats = torch.stack(sample_feats).squeeze(dim=0)
        # sample_adjs = torch.stack(sample_adjs).squeeze(dim=0)
        # sample_batch = Data(x=sample_feats, edge_index=sample_adjs)
        # return sample_batch
