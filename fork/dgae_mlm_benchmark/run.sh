...
# exp_name='reproduce_mlm_benchmark' #复现mlm结果
# exp_name='reproduce_mlm_benchmark_T20' #复现mlm结果
# exp_name='reproduce_mlm_benchmark_modify_pe' #复现mlm结果
exp_name='reproduce_dgae' #复现mlm结果
...
# if python main.py --work_type train_autoencoder --gpu $gpu --exp_name $exp_name --dataset $dataset --use_mask --wandb $wandb --codebook_size $codebook_size --nc $nc --quantization $quantization 2>&1| tee 'models_own/'$exp_name'/'$dataset'_autoencoder/'$date'.log'; then
if python main.py --work_type train_autoencoder --gpu $gpu --exp_name $exp_name --dataset $dataset --wandb $wandb --codebook_size $codebook_size --nc $nc --quantization $quantization 2>&1| tee "models_own/$exp_name/${dataset}_autoencoder/$date.log"; then
