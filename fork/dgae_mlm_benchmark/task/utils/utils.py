...
def split_ids(args, node_num):
    ...
    ids = {}
    ids['train'] = node_ids[:int(train_ratio * len(node_ids))] #前40%作为训练集
    ids['val'] = node_ids[int(train_ratio * len(node_ids)):int((train_ratio + val_ratio) * len(node_ids))] #40%-60%作为验证集
    ids['test'] = node_ids[int((train_ratio + val_ratio) * len(node_ids)):] #60%-100%作为测试集
...
def load_graph(args):
    ...
    with np.load(dataset, allow_pickle = True) as loader:
        ...
        graph = graph + graph.transpose() #对称矩阵
        if args.noise_num > 0: #add noise
            graph = graph + sp.identity(graph.shape[0]) #means add self-loop
        graph = sp.csr_matrix.toarray(graph) #转为dense matrix

        # Feature matrix
        if 'attr_data' in loader:
            # Attributes are stored as a sparse CSR matrix
            features = sp.csr_matrix((loader['attr_data'], loader['attr_indices'], loader['attr_indptr']), shape=loader['attr_shape']) #特征矩阵
            features = sp.csr_matrix.toarray(features) #转为dense matrix
            # Normalize
            features = normalize(features, axis=1, norm='l2') #对每一行进行归一化
            #features = features - np.mean(features, axis=0)
        elif 'attr_matrix' in loader:
            # Attributes are stored as a (dense) np.ndarray
            features = loader['attr_matrix'] #特征矩阵
        else:
            features = None
    ...
    labels = labels - labels.min() #将标签从0开始
