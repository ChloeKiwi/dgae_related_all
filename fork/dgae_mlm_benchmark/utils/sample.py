...
def sample_batch(n_samples, transformer, quantizer, decoder, T, use_mlm):
    with torch.no_grad():
        start = time.time()
        transformer.init_sampler(n_samples)
        zq, masks, indices = sample_prior(n_samples, transformer, quantizer, T=T, use_mlm=use_mlm, padding=not use_mlm) #生成的zq, masks, indices
        ...
        if adjs_recon.shape[-1] == 1:
            ...
            if annots_recon.shape[-1] == 0: #no node feat
                annots_recon = None
            else: #has node feat:ego data
                annots_recon = annots_recon.sigmoid().round() * masks
        # annots_recon = None #原版本
        else: #mol类数据
...
def sample(n_sample, transformer, quantizer, decoder, T, use_mlm):
    if n_sample <= 1000:
        return sample_batch(n_sample, transformer, quantizer, decoder, T, use_mlm)
    else:
        K = (n_sample // 1000) #means 1000个一个batch
        mod = n_sample % 1000  #means 剩下的个数
        annots, adjs = sample_batch(n_sample // K, transformer, quantizer, decoder, T, use_mlm) #第一个batch
        for k in range(K-1): #K-1个batch
            ann, adj = sample_batch(n_sample // K, transformer, quantizer, decoder, T, use_mlm) #剩下的K-1个batch
            annots = torch.cat((annots, ann), dim=0)
            adjs = torch.cat((adjs, adj), dim=0)
        if mod != 0:
            ann, adj = sample_batch(mod, transformer, quantizer,decoder, T, use_mlm) #剩下的mod个batch
            annots = torch.cat((annots, ann), dim=0)
            adjs = torch.cat((adjs, adj), dim=0)
    return annots, adjs
...
def sample_prior(n_samples, transformer, quantizer, T=10, use_mlm=True, padding=False): 
    if use_mlm:
        with torch.no_grad():
            transformer.eval()
            n_embeddings = quantizer.n_embeddings
            ...
            for t in range(T):
                # 获取所有位置的预测
                logits = transformer(use_mlm,cur_ids) #([20, 21, 1, 259]) torch.Size([20, 21, 2, 19])
            ...
            # 处理结果
            indices = cur_ids[:, 1:]  # 移除sos token
            zq = quantizer.indices_to_zq(indices, padded=True)
            mask = (indices[:, :, 0] != n_embeddings).unsqueeze(-1)
        ...
        with torch.no_grad():
            ...
            z_completed = torch.zeros(n_samples, 1, nc,  nv).to(device) #20,1,1,8
            indices = torch.zeros(n_samples, n_max, nc, dtype=torch.long).to(device) #20,20,1
