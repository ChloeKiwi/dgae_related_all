...
def get_data_info(config, data, dataset):
    ...
    if dataset == 'qm9' or dataset == 'zinc':
        ...
        data_info.annotated_nodes = False #图数据集的nodes没有特征
    ...
    if hasattr(data[0], 'edge_attr_ext'):
        ...
        # 如果没有边属性，设置为默认值1（仅表示连接关系）
        data_info.n_edge_feat = 1
    
    if data_info.n_node_feat != 0: #nodes有特征:ego data
        data_info.annotated_nodes = True #需要重构nodes特征
