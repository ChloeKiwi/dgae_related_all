...
class Trainer:
    def __init__(self, dataloaders, config, data_info):
        ...
        # self.quantization = False #测试vq是否必要实验
        # self.quantization = config.quantization if config.quantization is not None else False #todo 在run.sh中设置false时没用
        ...
        self.T = 10
        print(f"use_mask: {self.use_mask}, sos_token: {self.sos_token}, mask_token_id: {self.mask_token_id}, nc: {self.nc}, wandb:{config.log.wandb}, t:{self.T}")
        ...
        if config.work_type != 'sample':
            ...
            self.n_samples = batch.num_graphs #train模式batch有多少sample就生成多少sample
        else:
            self.n_samples = config.n_samples #统一117
        ...
        for epoch in tqdm(range(1, self.epochs)):
            for batch_id, batch in enumerate(self.train_loader):
                ...
                if step % self.n_logging_steps == 0:
                    log_running_metrics(self.metrics, self.wandb, global_step, key='iter', times=times)
                    if self.mol_data:
            ...
            for batch_id, batch in enumerate(self.test_loader):
                with torch.no_grad():
                    ...
                    if batch_id == len(self.test_loader)-2 and epoch%100==0 and not self.mol_data: 
                        ...
                        plot_graphs(adjs_recon[:20], max_plot=20, wandb=self.wandb, title=f'reconstruction data epoch {epoch}', filepath=f"./plots/{self.config.exp_name}", filename=f"{self.config.dataset}_recon_graph_test_{epoch}")
            
            if epoch % self.n_logging_epochs == 0:
                logged_metrics = log_running_metrics(self.metrics, self.wandb, global_step, key='train')
                to_save = (self.encoder, self.decoder, self.quantizer, self.opt, self.scheduler)
                self.best_run = save_model(logged_metrics['train/recon_loss'], self.best_run, to_save, global_step,
                ...
                logged_metrics = log_running_metrics(self.metrics, self.wandb, global_step, key='val', times=times)
                to_save = (self.encoder, self.decoder, self.quantizer, self.opt, self.scheduler)
                self.best_run = save_model(logged_metrics['val/recon_loss'], self.best_run, to_save, global_step,
            ...
            if global_step % self.decay_iteration == 0 and self.scheduler.get_last_lr()[0] > 2 * 1e-5:
                self.scheduler.step()
    ...
    def fit_autoencoder(self, batch: Data, train: bool = True) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.init_steps > 0:
            ...
            nodes_rec, edges_rec = log_step_autoencoder(self.metrics, batch, rec_losses, vq_losses, masks,
                                            self.n_node_feat, train, self.annotated_nodes,self.mol_data)
        ...
        for epoch in tqdm(range(1, self.epochs)):
            ...
            for batch in self.train_loader:
                ...
                if step % self.n_logging_steps == 0:
                    ...
                    annots, adjs = sample_batch(self.n_samples, self.transformer, self.quantizer, self.decoder, T=self.T, use_mlm=self.use_mask) #生成样本
            ...
            if epoch % self.n_logging_epochs == 0:
                ...
                annots, adjs = sample(self.n_samples, self.transformer, self.quantizer, self.decoder, T=self.T, use_mlm=self.use_mask)
                if self.mol_data:
                    ...
                    key = 'avg'
                    print(f"prior mmd at val epoch {epoch}: {gen_metrics}")
    ...
    def fit_prior(self, batch: Data, train: bool = True) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        ze, w0 = self.encoder(batch) #ze:n_nodes,nz
        # import ipdb; ipdb.set_trace()
        ...
        if self.config.use_mask and ignore_pad==True:
            ...
            loss = F.cross_entropy(
                ...
                ignore_index=self.mask_token_id            # 忽略 mask id, sos token
            )
...
def get_output_mask(bs: int, indices: torch.Tensor, cb_size: int, device: torch.device,
    ...
    # causal mask
    # tril = torch.tril(torch.full((cb_size + 1, cb_size + 1), float('-inf')), diagonal=-1).to(device) #+1:pad
    tril = torch.tril(torch.full((cb_size + 3, cb_size + 3), float('-inf')), diagonal=-1).to(device) #+3:pad,mask,sos
