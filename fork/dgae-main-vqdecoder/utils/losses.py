...
def get_losses_linear_decoder(batch, nodes_rec, edges_rec, node_masks, annotated_nodes,
               annotated_edges, max_node_num, n_node_feat, edge_masks):
    if annotated_nodes:
        node_loss, nodes_rec, nodes_target = get_node_loss_and_recon(batch, nodes_rec, node_masks,
                                                                     max_node_num, n_node_feat)
    ...
    if annotated_edges:
        ...
        edge_loss, edges_rec = get_unannotated_loss_and_rec_linear_decoder(batch, edges_rec, edge_masks, max_node_num)
...
def get_unannotated_loss_and_rec(batch, edges_rec, edge_masks, max_node_num):
    ...
    edges_rec shape: bs,n_max,n_max,dim
    '''
    edges_rec = (edges_rec.transpose(1, 2) + edges_rec) * .5
...
def get_unannotated_loss_and_rec_linear_decoder(batch, quantized_edge, edge_masks, max_node_num):
    """
    when linear decoder, edges_rec shape: bs,n_max,nz
    """
    adj_quantized = torch.matmul(quantized_edge, quantized_edge.permute(0,2,1)) #bs,n_max,n_max
    adj_quantized = (adj_quantized - adj_quantized.min()) / (adj_quantized.max() - adj_quantized.min()) #正则化
    adj_quantized = adj_quantized * edge_masks.squeeze(-1)
    edges_rec = adj_quantized.round() #四舍五入
    adjs = to_dense_adj(batch.edge_index, batch=batch.batch, max_num_nodes=max_node_num)
    edge_rec_loss =torch.sqrt(F.mse_loss(adjs, adj_quantized))
    return edge_rec_loss, edges_rec
