...
from utils.losses import get_losses, get_losses_linear_decoder
...
class Trainer:
    ...
    def recon_adj_from_decoder(self, nodes_rec, edges_rec, masks, edge_masks):
        ...
        adjs_recon = torch.matmul(edges_rec, edges_rec.permute(0,2,1))
        if adjs_recon.shape[-1] == 1 or adjs_recon.ndim==3:
    ...
    def fit_autoencoder(self, batch: Data, train: bool = True) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        nodes_rec, edges_rec = self.decoder(zq, mask=node_masks) #[32, 20, 0]; [32, 20, 20, 1]/[32,20,8]

        if edges_rec.ndim==3:
            recon_loss, rec_losses = get_losses_linear_decoder(batch, nodes_rec, edges_rec, node_masks.unsqueeze(-1), self.annotated_nodes,
