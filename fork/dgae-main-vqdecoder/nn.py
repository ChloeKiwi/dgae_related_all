...
class Gnn(nn.Module):
    ...
    def forward(self, node_feat, edge_feat=None, skip_connection=False):
        ...
        node_out = node_out.sum(-2) #聚合邻居节点信息
        if skip_connection:
        ...
        return node_out, edge_out #bs,n_max,node_out; bs,n_max,n_max,edge_out
...
#         # 2 linear decoder
#         self.edge_linear = Mlp(nnf_in, nnf_in, hidden)
#         self.node_linear = Mlp(nnf_in, nnf_out, hidden)

#         if normalization == 'batch_norm':
#             self.edge_norm = nn.BatchNorm1d(nnf_in)
#             self.node_norm = nn.BatchNorm1d(nnf_out)

#         elif normalization == 'layer_norm':
#             self.edge_norm = nn.LayerNorm(nnf_in)
#             self.node_norm = nn.LayerNorm(nnf_out)

#         self.normalization = normalization
        
#     def forward(self, node_feat, edge_feat=None, skip_connection=False):
#         # node_feat is the quantized vector
#         import ipdb; ipdb.set_trace()
#         quantized_edge = self.edge_linear(node_feat) #bs,n_max,nz
#         quantized_node = self.node_linear(node_feat) #bs,n_max,node_out
        
#         if self.normalization == 'batch_norm':
#             edge_out = self.edge_norm(quantized_edge.transpose(1,2)).transpose(1,2)
#             node_out = self.node_norm(quantized_node.transpose(1,2)).transpose(1,2)
#         elif self.normalization == 'layer_norm':
#             edge_out = self.edge_norm(quantized_edge)
#             node_out = self.node_norm(quantized_node)
#         return node_out, edge_out #bs,n_max,node_out ; bs,n_max,nz
