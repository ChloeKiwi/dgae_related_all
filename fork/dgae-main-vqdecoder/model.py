...
class Decoder(nn.Module):
    ...
    def forward(self, node_feat, mask=None): #node_feat: bs,n_max,nz
        node_feat, edge_feat = self.layers[0](node_feat) #32,20,32; 32,20,20,32
        ...
        node_feat, edge_feat = self.layers[-1](node_feat, edge_feat, skip_connection=False) #32,20,0 tensor([]); 32,20,20,1
        if mask is not None:
            node_feat = node_feat * mask.unsqueeze(-1)
            edge_feat = edge_feat * mask.reshape(node_feat.shape[0], -1, 1, 1)
            edge_feat = edge_feat * mask.reshape(node_feat.shape[0], 1, -1, 1)
        return node_feat, edge_feat #32, 20, 0; [32, 20, 20, 1])
...
class Decoder(nn.Module):
    def __init__(self, config, data_info):
        ...
        self.edge_layers = nn.ModuleList([nn.Linear(nz, mlp_hidden_size)])  # 使用 ModuleList 而不是普通的 list
        for layer in range(1, n_layers - 1):
            self.edge_layers.append(nn.Linear(mlp_hidden_size, mlp_hidden_size))
        self.edge_layers.append(nn.Linear(mlp_hidden_size, nhf))
        
        self.node_layers = nn.ModuleList([nn.Linear(nz, mlp_hidden_size)])  # 使用 ModuleList 而不是普通的 list
        for layer in range(1, n_layers - 1):
            self.node_layers.append(nn.Linear(mlp_hidden_size, mlp_hidden_size))
        self.node_layers.append(nn.Linear(mlp_hidden_size, nnf))

        if normalization == 'batch_norm':
            self.edge_norm = nn.BatchNorm1d(mlp_hidden_size)
            self.node_norm = nn.BatchNorm1d(mlp_hidden_size)
        elif normalization == 'layer_norm':
            self.edge_norm = nn.LayerNorm(mlp_hidden_size)
            self.node_norm = nn.LayerNorm(mlp_hidden_size)
        
    def forward(self, node_feat, mask=None): #mask: bs,n_max
        node_out = self.node_norm((self.node_layers[0](node_feat)).transpose(1,2)).transpose(1,2)
        edge_out = self.edge_norm((self.edge_layers[0](node_feat)).transpose(1,2)).transpose(1,2)
        
        for i in range(1, self.n_layers-1):
            node_out = self.node_norm((self.node_layers[i](node_out)).transpose(1,2)).transpose(1,2)
            edge_out = self.edge_norm((self.edge_layers[i](edge_out)).transpose(1,2)).transpose(1,2)
        
        node_out = self.node_layers[-1](node_out) #bs,n_max,nnf
        edge_out = self.edge_layers[-1](edge_out) #bs,n_max,nhf
        
        if mask is not None:
            node_out = node_out * mask.unsqueeze(-1)
            edge_out = edge_out * mask.unsqueeze(-1)
            
        return node_out, edge_out #bs,n_max,node_out ; bs,n_max,nhf
