...
class Encoder(nn.Module):
    ...
    def forward(self, batch):
        ...
        node_feat, edge_feat = self.layers[0](batch.x,
                                              ...
                                              edge_attr=batch.edge_attr_ext if hasattr(batch,'edge_attr_ext') else None)
        for i, layer in enumerate(self.layers[1:-1]):
...
class ClassificationHead(nn.Module):
    def __init__(self, config):
        ...
        self.mlp = nn.Sequential(
            nn.Linear(config.nz, config.nz*2),
            nn.ReLU(),
            nn.Dropout(p=0.2),
            nn.Linear(config.nz * 2, config.nz),
            nn.ReLU(),
            nn.Dropout(p=0.2),
            nn.Linear(config.nz, config.class_num)
        )
...
class Quantizer(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.class_num = config_all.n_node_class if hasattr(config_all, 'n_node_class') else config_all.n_graph_class
        config.class_num = self.class_num
        # self.linear = nn.Linear(config.nz, self.class_num)
        self.linear = ClassificationHead(config)
    ...
    def forward(self, x, mask=None):
        ...
        # compute classify logits
        logits = self.linear(quantized)
        
        return quantized, commitment_loss, codebook_loss, perplexity, indices, logits
