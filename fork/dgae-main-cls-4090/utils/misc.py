...
#### code is from grapher ####

def graph_to_file(G, filename):
    G.remove_edges_from(nx.selfloop_edges(G)) # Remove self-loops from the graph
...
def to_latex_row(values, stds=None): # 将给定的值 values 和可选的标准差 stds 格式化为 LaTeX 行
...
def to_latex_table(all_values, stds_all=None): #将所有值 all_values 和可选的标准差 stds_all 格式化为 LaTeX 表格
    if stds_all is None:
