...
class Trainer:
    def __init__(self, dataloaders, config, data_info):
        ...
        if hasattr(data_info, 'n_node_class'):
            config.n_node_class=data_info.n_node_class 
        elif hasattr(data_info, 'n_graph_class'):
            config.n_graph_class=data_info.n_graph_class
        ...
        with torch.no_grad():
            for batch in tqdm(self.test_loader, desc='Collecting embeddings'):
                ...
                if self.quantization:
                    zq, _, _, _, _, _ = self.quantizer(ze)
                else:
                    zq = ze
    ...
    def fit_autoencoder(self, batch: Data, train: bool = True) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.init_steps > 0:
            ...
            if self.quantization:
                zq, commit, codebook, perplex, indices, logits = self.quantizer(ze) #logits:[bs,n_node,n_class]
        ...
        # classification loss
        # logits:n_node,n_class
        # labels = batch.label.long()
        # cls_weight = 1.0  # 可调整的权重系数
        cls_weight = 0  # 可调整的权重系数
        cls_loss = cls_weight * F.cross_entropy(logits, labels)
        
        if self.init_steps>0:
            loss = recon_loss
        else:
            # loss = recon_loss + (codebook + commit) * self.gamma
            loss = recon_loss + (codebook + commit) * self.gamma + cls_loss
        ...
        if self.init_steps > 0:
            ...
            vq_losses = loss, recon_loss, codebook, commit, perplex, cls_loss, acc
