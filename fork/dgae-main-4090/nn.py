...
class Gnn(nn.Module):
    ...
    def forward(self, node_feat, edge_feat=None, skip_connection=False):
        ...
        if edge_feat is not None:
            ...
            edge_feat = nodes2nodes #bs,n_max,n_max,2dim
...
def nodes2edges(nodes): #bs,n_max,dim
    nodes_ = nodes.unsqueeze(3).permute(0, 3, 1, 2) #bs,1,n_max,dim
    nodes_ = nodes_.repeat(1, nodes.shape[1], 1, 1) #bs,n_max,n_max,dim
    nodesT = nodes_.transpose(1, 2) #bs,n_max,n_max,dim
    return torch.cat([nodes_, nodesT], dim=3) #bs,n_max,n_max,2*dim
