...
class DiffLoss(nn.Module):
    """Diffusion Loss"""
    def __init__(self, target_channels, z_channels, depth, width, num_sampling_steps, device, grad_checkpointing=False):
    ...
    def forward(self, target, z, mask=None, node_mask=None):
        t = torch.randint(0, self.train_diffusion.num_timesteps, (target.shape[0],), device=self.device)
        model_kwargs = dict(c=z)
        loss_dict = self.train_diffusion.training_losses(self.net, target, t, model_kwargs)
        loss = loss_dict["loss"]
        # 同时应用两个mask进行过滤
        if mask is not None and node_mask is not None:
            # 两个mask相乘得到最终的mask
            final_mask = mask * node_mask
            # 使用最终mask对loss进行过滤
    ...
    def sample(self, z, temperature=1.0, cfg=1.0):
        ...
        return sampled_token_latent #bs, 8 
...
class SimpleMLPAdaLN(nn.Module):
    ...
    def forward(self, x, t, c):
        ...
        """
        x = self.input_proj(x)
        t = self.time_embed(t.to(x.device))
        c = self.cond_embed(c.to(x.device))
