...
class Trainer:
    def __init__(self, dataloaders, config, data_info):
        ...
        self.encoder, self.decoder, self.quantizer, self.transformer, self.mar, self.opt, self.scheduler = model
                ...
                with torch.no_grad():
                    ...
                    if batch_id == len(self.test_loader)-2 and epoch%10==0: 
                        if not self.mol_data:
                            plot_graphs(adjs_recon[:20], max_plot=20, wandb=self.wandb, title=f'reconstruction data epoch {epoch}', 
        ...
        for epoch in tqdm(range(1, 10000)):
            for batch in self.train_loader:
                ...
                if step % self.n_logging_steps == 0:
                    ...
                    annots, adjs = sample_batch(self.n_samples, self.mar, self.quantizer, self.decoder) #生成样本

                    if self.mol_data:
                        gen_metrics = log_mol_metrics(annots, adjs, self.dataset, key='prior train')
                        key = 'nspdk'
                        to_save = (self.mar, self.opt, self.scheduler)
            ...
            if epoch % self.n_logging_epochs == 0:
                ...
                annots, adjs = sample(self.n_samples, self.mar, self.quantizer, self.decoder)
                if self.mol_data:
    ...
    def fit_prior(self, batch: Data, train: bool = True) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if train:
            self.opt.zero_grad()
            # self.transformer.train() #discrete ar model 
            self.mar.train() #continous ar model 
        else:
        ...
        ze, w0 = self.encoder(batch) #ze:num_node,nc*nz
        
        # todo: MAE implementation
        # x = ze #num_node,nc*nz
        # dense_x, node_mask = to_dense_batch(x, batch.batch, max_num_nodes=self.max_node_num) #bs,n_max,nc*nz; bs,n_max        
        # diff_loss = self.mar(dense_x, labels=None, node_mask=None) #输入continous latents #todo conditioned-label version; 同时学习pad位置
        # loss = diff_loss
        
        # todo: mlm+diffloss implemention
        ae_latents = ze 
        latents, node_mask = to_dense_batch(ae_latents, batch.batch, max_num_nodes=self.max_node_num) #bs,n_max,nc*nz; bs,n_max    
        latents = latents.reshape(-1, self.max_node_num, self.nc, self.quantizer.embedding_dim) #bs,n_max,nc,nz
                
        # quantizer
        zq, indices = self.quantizer.encode(ze) #indices:正确索引(bs,nc)，zq:量化后的节点特征(bs,nc,nz)
        zq, egde_masks, indices = dense_zq(zq, indices, self.quantizer, batch, self.max_node_num, self.sort_indices)
        # #zq:(bs,max_node_num,nc,nz),masks:(max_node_num,max_node_num),indices:(bs,max_node_num,nc)
        
        ignore_pad = False
        mask_latents = False
        mask_indices = not mask_latents
        if self.config.use_mask and ignore_pad and mask_indices:
        ...
        elif self.config.use_mask and not ignore_pad and mask_indices:
            bs, seq_len, nc = indices.shape
            # mask = torch.zeros_like(indices, dtype=torch.bool)
            ...
            mask = mask.unsqueeze(-1)  # (bs, seq_len, nc, 1)
            mask = mask.expand(-1, -1, -1, self.quantizer.embedding_dim)  # (bs, seq_len, nc, nz)
            # 由于zq的pad位置向量为0,所以mask设为全0.1向量,后续diffloss会mask掉
            mask_emb = torch.ones_like(latents) * 0.1 
            masked_latents = torch.where(mask, mask_emb, latents) #(32,20,1,8)
            logits = self.transformer(a_indices, latents=masked_latents) #(32,20,1,64)
            
            # #不能预测sos1025、mask1026，只能预测0-1024（pad）
            # logits = self.transformer(a_indices) #(32,20,1,257)
            
            # # 1 celoss
            # ce_loss = F.cross_entropy( #不计算sos、mask
            ...
            #! 2 diffloss
            diff_loss = self.mar(logits, labels=None, node_mask=node_mask, use_mlm=True, target_latents=masked_latents, mask=mask) #输入continous latents 
            loss = diff_loss
                    
        else:
            # causal transformer
            logits = self.transformer(zq, padding_mask=masks) #auto regressive training
