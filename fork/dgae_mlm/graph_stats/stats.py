...
# def degree_stats(graph_ref_list, graph_pred_list, is_parallel=True, max_workers=4):
#     ''' Compute the distance between the degree distributions of two unordered sets of graphs.
#     Args:
#       graph_ref_list, graph_target_list: two lists of networkx graphs to be evaluated
...
#         if is_parallel:
#             # 限制线程池大小
#             n_workers = min(os.cpu_count(), max_workers) if max_workers else os.cpu_count()
...
#             print(f"Warning: Failed to compute MMD with gaussian_emd ({str(e)}), trying gaussian kernel")
#             mmd_dist = compute_mmd(sample_ref, sample_pred, kernel=gaussian)
