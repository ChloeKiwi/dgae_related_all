...
def sample_batch(n_samples, transformer, quantizer, decoder, max_node_num):
    with torch.no_grad():
        ...
        zq, _, indices = sample_prior(n_samples, transformer, quantizer, T=max_node_num//2) #生成的zq, masks, indices
...
def sample_prior(n_samples, transformer, quantizer, T=10): 
    with torch.no_grad():
        ...
        file_path = os.path.join("models_own", "indice_mlm_baseline", "indices.txt")
