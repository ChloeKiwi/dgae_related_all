...
def sample_batch(n_samples, transformer, quantizer, decoder, max_node_num):
    with torch.no_grad():
        ...
        zq, node_masks, indices = sample_prior_nar_vanilla(n_samples, transformer, quantizer, T=max_node_num//2)
...
# maskgit + predict eos
def sample_prior_nar_vanilla(n_samples, transformer, quantizer, T=None): 
    with torch.no_grad():
        # transformer.eval()
        transformer.train()
        n_embeddings = quantizer.n_embeddings
        ...
        for t in range(T if T is not None else n_max):
            ...
            # 在最后一轮迭代后，确保序列有正确的结构
            # if t == T-1:
            #     # 找到每个序列中最后一个非pad token的位置
            #     last_token_pos = torch.max((cur_ids[:, 1:] != n_embeddings).float().argmax(dim=1), dim=1)[0]
                
            #     # 在该位置后添加EOS token（如果还没有）
            #     for b in range(n_samples):
            #         pos = last_token_pos[b].item() + 1
            #         if pos < n_max and cur_ids[b, pos, 0] != transformer.eos_token:
            #             cur_ids[b, pos] = transformer.eos_token
            #             if pos + 1 < n_max + 1:
            #                 cur_ids[b, pos+1:] = n_embeddings
