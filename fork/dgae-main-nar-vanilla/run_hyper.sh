...
declare -A experiment_configs=(
    ...
    ['community_128_1_not_ignore_pad']="community 128 1 2"
...
run_experiment() {
    ...
    # if python main.py --work_type train_prior --gpu $gpu --exp_name $exp_name --run_name $exp_config --dataset $dataset --wandb $wandb --codebook_size $codebook_size --nc $nc --use_mask 2>&1 | tee "models_own/$exp_name/$exp_config/${dataset}_prior/$date.log"; then
    ...
    python main.py --work_type sample --gpu $gpu --exp_name $exp_name --run_name $exp_config \
        --dataset $dataset --codebook_size $codebook_size --nc $nc --use_mask \
