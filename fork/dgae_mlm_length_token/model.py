...
class Transformer2d(nn.Module):
    def __init__(self, config):
        ...
        out_dim = config.model.quantizer.codebook_size + 3 #+3 for pad(256), mask, sos tokens/length token
        ...
        # self.length_token = self.sos_token #todo: 用length_token替代sos_token

        # 新增：token embedding层
        self.tok_emb = nn.Embedding(
            config.model.quantizer.codebook_size + 3,  # +3 for pad(256), sos(257), mask(258)
        ...
        self.drop = nn.Dropout(p=config.training.drop)
        ...
        # # 新增: length token的embedding层
        # self.length_token = nn.Parameter(torch.randn(1, 1, d_model))
        # self.length_predictor = nn.Linear(d_model, n_max)
        self.embed_lengths = nn.Embedding(n_max+1, d_model) #
    ...
    def forward(self, indices, padding_mask=None):
        ...
        # 1. 准备length token
        # length_embeddings = self.length_token.expand(bs, 1, self.d_model)  # (bs, 1, d_model)
        length_embeddings = self.embed_lengths(indices.new(bs, 1).fill_(0)) #(bs,1,d_model)
        # 2. 处理每个通道
        for c in range(nc):
            ...
            # 使用可训练的embedding层
            z[c] = self.tok_emb(indices[..., c])  # (bs, n_max+1, d_model)
            # 添加位置编码
            z[c] = z[c] + self.pe(z[c])
            # 将length token与输入序列拼接
            z[c] = torch.cat([length_embeddings, z[c]], dim=1)  # (bs, n_max+2, d_model)
            
            # 如果有padding_mask，将pad位置的embedding设为0
            if padding_mask is not None:
                # 为length token创建mask (全1，表示始终参与注意力计算)
                length_mask = torch.ones(bs, 1, device=device)
                # 拼接padding mask
                extended_mask = torch.cat([length_mask, padding_mask[..., c]], dim=1) #(bs,n_max+2,nc)
                z[c] = z[c] * extended_mask.unsqueeze(-1) #todo: embedding层的pad位置已经初始化为0，这里添加了length token后，pad位置仍然为0
        ...
        v_seq = v[:,1:,:] #remove length token
        v_seq = self.pe(v_seq) #(bs,n_max+1,d_model) 
        v = torch.cat([v[:,0,:].unsqueeze(1), v_seq], dim=1) #(bs,n_max+2,d_model) add length token
        ...
        # 从length token的输出预测序列长度
        length_logits = torch.matmul(emb[0][:, 0], self.embed_lengths.weight.transpose(0, 1)).float()  # 根据单独的length params预测长度 (bs,n_max)
        # length_logits = self.length_predictor(emb[0][:, 0]).float()  # 使用第一个位置的输出 (bs,n_max)
        length_logits[:,0] += float('-inf') # 0长度位置为无穷小        
        
        # 5. 输出层
        for c in range(nc):
            # emb[c] = self.mlp_out[c](emb[c])
            emb[c] = self.mlp_out[c](emb[c][:,1:]) #去掉length token
        
        out = torch.stack(emb, dim=-2) #(32,21,1,257)
        return out, length_logits #out包含sos token
