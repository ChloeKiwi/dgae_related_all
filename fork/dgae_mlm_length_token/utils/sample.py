...
def sample_batch(n_samples, transformer, quantizer, decoder, max_node_num):
    with torch.no_grad():
        ...
        zq, node_masks, indices = sample_prior(n_samples, transformer, quantizer, T=max_node_num//2) #生成的zq, masks, indices
        zq = quantizer.indices_to_zq(indices.long(), padded=True) #(bs,max_node_num,nc,nz)
        masks = node_masks.unsqueeze(-1) #(bs,max_node_num,1)
...
def sample_prior(n_samples, transformer, quantizer, T=10): 
    with torch.no_grad():
        ...
        for t in range(T):
            ...
            logits, length_logits = transformer(cur_ids) #([20, 21, 1, 259]) torch.Size([20, 21, 2, 19]) (n_samples, max_node_num, nc, cb_size+3)
            if t==T-1:
                predicted_lengths = torch.argmax(length_logits, dim=-1) #预测最后一次长度 (bs,)
            ...
            if cur_ids.size(2)==1:
                cur_ids = cur_ids.squeeze(-1) # 需要赋值回去，否则squeeze不会改变原tensor
            cur_res = 'iter' + str(t) + ": " + str(cur_ids[0].cpu().numpy()) 
        ...
        # 根据predicted_lengths，mask掉length以后的位置
        mask = torch.zeros(n_samples, n_max, nc, device=device) #(bs, max_node_num, nc)
        len_mask = torch.arange(n_max, device=device) < predicted_lengths.unsqueeze(1) #(bs,max_node_num)
        mask = len_mask #(bs,max_node_num)
        
        # 将predicted id写入文件方便观察
        file_path = os.path.join("models_own", "indices_res_length", "indices.txt")
...
#     else:
#         beam = predicted_lengths.topk(length_beam_size, dim=1)[1] #选择最有可能的length_beam_size个长度
#     beam[beam < 2] = 2
#     return beam
