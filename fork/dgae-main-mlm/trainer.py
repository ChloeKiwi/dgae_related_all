...
class Trainer:
    def __init__(self, dataloaders, config, data_info):
        ...
        if config.work_type != 'sample':
            ...
            wandb.init(project=f'VQ-GAE_{config.dataset}', config=config, mode=config.log.wandb, 
                       name=f'{config.exp_name}_{config.run_name}_{config.work_type}', id=run_id, resume=True)
            wandb.save("*.pt")
    ...
    def fit_prior(self, batch: Data, train: bool = True) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        ignore_pad = False
        mask_latents = False
        mask_indices = not mask_latents #true
        if self.config.use_mask and not ignore_pad and mask_indices:
