...
# from graph_stats.mmd import process_tensor, compute_mmd, gaussian, gaussian_emd, compute_nspdk_mmd
from graph_stats.mmd import process_tensor, compute_mmd, gaussian, gaussian_emd
PRINT_TIME = True
...
def eval_torch_batch(ref_batch, pred_batch, methods=None):
    graph_ref_list = adjs_to_graphs(ref_batch.detach().cpu().numpy()) #转换成list
    ...
    results_novel_unique = eval_graph_novel_unique(graph_ref_list, graph_pred_list)
    return results, results_novel_unique

# def compute_novelty(graph_ref_list, graph_pred_list):
#     """计算生成图的新颖性"""
#     min_distances = []
#     for pred_g in graph_pred_list:
#         distances = []
#         for ref_g in graph_ref_list:
#             # 使用图编辑距离作为相似度度量
#             distance = nx.graph_edit_distance(pred_g, ref_g, timeout=1)
#             if distance is not None:  # 如果超时返回None则忽略
#                 distances.append(distance)
#         if distances:
#             min_distances.append(min(distances))
#     return np.mean(min_distances) if min_distances else 0.0

# def compute_uniqueness(graph_pred_list):
#     """计算生成图的独特性"""
#     if len(graph_pred_list) <= 1:
#         return 1.0
    
#     distances = []
#     for i in range(len(graph_pred_list)):
#         for j in range(i + 1, len(graph_pred_list)):
#             distance = nx.graph_edit_distance(graph_pred_list[i], graph_pred_list[j], timeout=1)
...
def eval_graph_novel_unique(graph_ref_list, graph_pred_list):
    results = {}
    results['novelty'] = round(novelty(graph_ref_list, graph_pred_list, fast=True), 6)
    results['uniqueness'] = round(uniqueness(graph_pred_list, fast=True), 6)
    print('\033[91m' + f'{"novelty":9s}' + '\033[0m' + ' : ' + '\033[94m' +  f'{results["novelty"]:.6f}' + '\033[0m')
    print('\033[91m' + f'{"unique":9s}' + '\033[0m' + ' : ' + '\033[94m' +  f'{results["uniqueness"]:.6f}' + '\033[0m')
    return results