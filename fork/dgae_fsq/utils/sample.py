...
def sample_batch(n_samples, transformer, quantizer, decoder):
    with torch.no_grad():
        ...
        zq, masks, indices = sample_prior(n_samples, transformer, quantizer, T=10) #生成的zq, masks, indices
...
def sample_prior(n_samples, transformer, quantizer, T=30): 
    with torch.no_grad():
        transformer.eval()
        n_embeddings = quantizer.n_embeddings
        ...
        for t in range(T):
            # 获取所有位置的预测
            logits = transformer(cur_ids) #([20, 21, 1, 259]) torch.Size([20, 21, 2, 19])
