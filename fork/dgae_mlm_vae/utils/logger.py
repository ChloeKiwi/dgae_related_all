...
def log_step_autoencoder(metrics, batch, rec_losses, vq_losses, masks, n_node_feat, train, annotated_nodes,mol_data=False, use_vae=True):
    ...
    if not use_vae:
        ...
        loss, recon_loss, codebook, commit, perplex, codebook_usage, kl_loss, vae_recon_loss = vq_losses
    ...
    if not use_vae:
        ...
        metric_values = (loss.item(), edge_loss.item(), edge_acc.item(), n_graph_rec.item(),
                        codebook.item(), commit.item(), recon_loss.item(),perplex.item(), codebook_usage, kl_loss.item(), vae_recon_loss.item())
