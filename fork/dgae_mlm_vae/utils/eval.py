...
def init_autoencoder_running_metrics(annotated_nodes):
    metric_names = ['loss', 'edge_loss', 'edge_acc', 'edge_acc',
                    'graph_rec', 'commit', 'codebook', 'recon_loss','perplexity', 'codebook_usage', 'kl_loss', 'vae_recon_loss']
    if annotated_nodes:
...
def reconstruction_stats(batch, edges_rec, nodes_rec, masks_nodes, masks_edges, n_node_feat,mol_data=True):
    '''
    Args:
    ...
    if nodes_rec is not None and mol_data: #mol data特征需要argmax分类
        max_node_num = nodes_rec.shape[1]
        dense_nodes, _ = to_dense_batch(batch.x, batch.batch, max_num_nodes=max_node_num) #原图的节点特征也进行dense
    ...
    elif nodes_rec is not None and not mol_data: #ego data特征是词袋，nodes_rec已经是1/0标识
        max_node_num = nodes_rec.shape[1] #64*18*3704
        dense_nodes, _ = to_dense_batch(batch.x, batch.batch, max_num_nodes=max_node_num) #原图的节点特征也进行dense
        nodes_true = dense_nodes[:, :, :n_node_feat]
        nodes_rec = nodes_rec[:, :, :n_node_feat]
        nodes_rec[(1 - masks_nodes.int()).bool()] = 0 
        correct_nodes = nodes_rec == nodes_true
        n_nodes_corr = correct_nodes.sum(-1) #64*18
        all_nodes_corr = n_nodes_corr == nodes_rec.shape[-1] #(n_samples, n_nodes)

        n_potential_nodes = nodes_rec.shape[0]*nodes_rec.shape[1] #1152
        n_nodes = masks_nodes.sum() #439
        
        n_nodes_corr = n_nodes_corr == n_node_feat #特征全对的节点数 (64,18)
        err_nodes = n_potential_nodes - n_nodes_corr.sum()
        err_nodes_rate = err_nodes/n_nodes
        acc_nodes = 1-err_nodes_rate
        
    if edges_rec.ndim==3:
        print("重构边还是3维度")
    if edges_rec.shape[-1] > 1 and edges_rec.ndim==4: #如果边特征的维度大于1
        edges_rec = edges_rec.argmax(-1) #边属性分类
    elif edges_rec.shape[-1] ==1 and edges_rec.ndim==4:
        edges_rec = edges_rec.round() #边连接分类
    edges_rec[(1-masks_edges).squeeze().bool()] = 0 #将重构的边特征中不属于原图的边特征设置为0

    edge_rec = edges_rec.squeeze() == edges_true

    n_edges_corr = edge_rec.sum([1, 2]) #(n_samples,)
    ...
    if nodes_rec is not None:
        ...
        if all_nodes_corr.ndim == 2:
            all_nodes_corr = all_nodes_corr.all(dim=1) #每个图的所有节点正确，才算正确
...
def get_mol_metric(gen_mols, dataset, num_no_correct, train_smiles=None):
    ...
    test_graph_list = mols_to_nx(test_mols)
