...
def load_smiles(dataset='QM9'):
    ...
    if dataset == 'QM9': #如果数据集是QM9，
        test_idx = test_idx['valid_idxs'] # 测试集的索引
        test_idx = [int(i) for i in test_idx] # 测试集的索引
...
def construct_mol(x, adj, atomic_num_list): # x: 9, 5; adj: 4, 9, 9
    mol = Chem.RWMol() # 可修改的分子对象

    atoms = np.argmax(x, axis=1) #9个 每个位置的原子类型
    atoms_exist = (atoms != len(atomic_num_list) - 1) # 9个 每个位置的原子是否存在
    #atoms_exist = x.sum(-1) != 0
    atoms = atoms[atoms_exist]             # 9,
    for atom in atoms:
        mol.AddAtom(Chem.Atom(int(atomic_num_list[atom]))) # 添加原子到分子中
