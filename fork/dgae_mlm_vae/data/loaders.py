...
class FromNetworkx(InMemoryDataset):
    ...
    def process(self):
        ...
        data_list = []
        for g in graph_list:
            data = from_networkx(g) #ego:Data(edge_index=[2, 20], feature=[6, 3703], label=[6], num_nodes=6)
            
            # # 处理节点特征
            if 'feature' in next(iter(g.nodes(data=True)))[1]:
                x = torch.FloatTensor([g.nodes[node]['feature'] for node in g.nodes()])
                data.x = x
            
            # # 处理节点标签
            if 'label' in next(iter(g.nodes(data=True)))[1]:
                node_labels = torch.LongTensor([g.nodes[node]['label'] for node in g.nodes()])
                data.y = node_labels
                                
            # 处理图标签（如果有的话）
            if hasattr(g, 'graph_label'):
                data.graph_label = torch.tensor([g.graph_label])
            
            # # 添加adj
            # import ipdb; ipdb.set_trace()
            # if hasattr(data, 'edge_index_ext'):
            #     data['adj'] = to_dense_adj(data.edge_index_ext, batch=data.batch)
            # else:
            #     data['adj'] = to_dense_adj(data.edge_index, batch=data.batch)
