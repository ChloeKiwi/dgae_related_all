...
class Gnn(nn.Module):
    ...
    def forward(self, node_feat, edge_feat=None, skip_connection=False):
        ...
        if edge_feat is not None:
            ...
            edge_feat = nodes2nodes #b,n,n,d ([i,j]i到j节点之间的边连接情况)
...
def nodes2edges(nodes): #nodes:(bs,n_nodes,node_dim)
    nodes_ = nodes.unsqueeze(3).permute(0, 3, 1, 2) # [b, 1, n, d]
    nodes_ = nodes_.repeat(1, nodes.shape[1], 1, 1) # [b, n, n, d]
    nodesT = nodes_.transpose(1, 2) # [b, n, n, d] n*n个节点的feat
    return torch.cat([nodes_, nodesT], dim=3)  # [b, n, n, 2d]
